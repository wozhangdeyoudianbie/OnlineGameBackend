...
namespace
{
    ...
    constexpr const char *kLookupSql = "SELECT create_request_id, state FROM matches WHERE match_id = ?";
    bool close_statement(MYSQL *connection, MYSQL_STMT *statement)
    {
        ...
        std::cerr << "[错误] 关闭 MySQL预处理语句失败：" << mysql_error(connection) << '\n';
        return false;
    }
    ...
    int lookup_match_by_id(MYSQL *connection, const std::string &match_id)
    {
        ...
        std::string request_storage = match_id;
        ...
        std::array<char, 64> create_request_id{};
        unsigned long create_request_id_length = 0;
        bool create_request_id_is_null = false;
        bool create_request_id_error = false;
        unsigned char state = 0;
        bool state_is_null = false;
        bool state_error = false;
        MYSQL_BIND result_bind[2]{};
        result_bind[0].buffer_type = MYSQL_TYPE_STRING;
        result_bind[0].buffer = create_request_id.data();
        result_bind[0].buffer_length = static_cast<unsigned long>(create_request_id.size());
        result_bind[0].length = &create_request_id_length;
        result_bind[0].is_null = &create_request_id_is_null;
        result_bind[0].error = &create_request_id_error;
        ...
        else if (create_request_id_is_null || state_is_null || create_request_id_error || state_error || create_request_id_length > create_request_id.size())
        {
        ...
        {
            const std::string create_request_id_value(create_request_id.data(), static_cast<std::size_t>(create_request_id_length));
            std::cout << "[找到] 创建请求ID=" << create_request_id_value << " 状态=" << static_cast<unsigned int>(state) << '\n';
        }
...
int main(int argc, char *argv[])
{
    ...
    const int result = lookup_match_by_id(connection, argv[1]);
    mysql_close(connection);
    mysql_library_end();
    return result;
}
